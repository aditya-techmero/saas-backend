...
import os
import json
# Load environment variables from .env for CLI execution
...
def generate_introduction_section_with_research(
    ...
    try:
        ...
        print(f"Just checkingout mate if its running or not !!!!!!!!!!")
        prompt = f"""
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate a blog post from the command line")
    parser.add_argument("title", help="Title of the blog post")
    parser.add_argument("--keywords", help="Comma-separated keywords to include")
    parser.add_argument("--research", action="store_true", help="Enable web research for the blog post")
    parser.add_argument("--tone", default="informative", help="Tone of the blog post")
    parser.add_argument("--sections", type=int, default=5, help="Number of main sections to generate")
    parser.add_argument("--proofread", action="store_true", help="Enable proofreading in post-processing")
    parser.add_argument("--humanize", action="store_true", help="Enable humanizing in post-processing")
    parser.add_argument("--output", default=None, help="Output markdown filename")
    args = parser.parse_args()

    keywords = args.keywords.split(",") if args.keywords else None
    # Create generation options (proofread/humanize are applied post-generation)
    options = GenerationOptions()
    if args.research:
        post = generate_blog_post_with_research(
            args.title, keywords, args.sections, True, args.tone, "openai", options
        )
    else:
        post = generate_blog_post(
            args.title, keywords, args.sections, True, args.tone, "openai", options
        )
    # Apply post-processing (proofread/humanize)
    post = post_process_blog_post(post, proofread=args.proofread, humanize=args.humanize)

    # Convert BlogPost object to markdown
    def to_markdown(bp):
        md = f"# {bp.title}\n\n{bp.description}\n\n"
    ...
    md = to_markdown(post)
    # Determine output path (default to content/blog/)
    default_name = f"{args.title.replace(' ', '_').lower()}.md"
    filename = args.output or os.path.join("content", "blog", default_name)
    # Ensure directory exists
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    with open(filename, "w") as f:
        f.write(md)
    print(f"Blog post generated and saved to {filename}")
